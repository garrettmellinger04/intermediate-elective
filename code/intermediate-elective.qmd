...
# create a separate dataset for label positions
label_data <- grass_plot

# calculate the total number of bars, including the empty bars
number_of_bar <- nrow(label_data)

# calculate the angle for each species label
angle <- 90 - 360 * (label_data$id - 0.5) / number_of_bar

# adjust label alignment based on where the label falls around the circle
label_data$hjust <- ifelse(angle < -90, 1, 0)

# flip labels on the bottom half of the circle so they are readable
label_data$angle <- ifelse(angle < -90, angle + 180, angle)

# create one color for each grass species
palette <- colorRampPalette(brewer.pal(11, "Spectral"))(
  length(unique(na.omit(grass_plot$psoc)))
)
...
p <- ggplot(grass_plot) +
  ...
  # create empty space inside the circle for title text
  ylim(
    -5,
    max(grass_plot$occurrence_count, na.rm = TRUE) + 5
  ) +
  ...
  theme(
    ...
    # add a title in the center of the circular plot
  annotate(
    geom = "text",
    x = 0,
    y = -4,
    hjust = 0.5,
    vjust = 1,
    label = "n Observation of Grass Species in the NCOS Mesa grassland",
    size = 4.3,
    lineheight = 0.8,
    fontface = "bold",
    color = "black"
  ) +
  # add a short explanation of what bar height means
  annotate(
    geom = "text",
    x = 0,
    y = max(grass_plot$occurrence_count, na.rm = TRUE) + 4,
    hjust = 0.5,
    vjust = 1,
    label = "Bar height shows number of observations per grass species",
    size = 2.8,
    color = "black"
  ) +
   # add species names around the outside of the circle
  geom_text(
    data = label_data,
    aes(
      x = id,
      y = occurrence_count + 1,
      label = psoc,
      hjust = hjust
    ),
    color = "black",
